...
function formCard(state, { onScan, onSubmit, onReboot }) {
  ...
  form.addEventListener('submit', (e) => { e.preventDefault(); onSubmit(form, submitBtn); });
  return el('div', { class: 'card' }, [
    el('h2', { class: 'card__title' }, '网络设置'),
    el('p',  { class: 'card__desc' },  '保存后可能需要重启以切换网络模式；点「重启设备」立即生效。'),
    form,
  ]);
}

async function handleSubmit(form, btn) {
  ...
  try {
    ...
    toast(d.reboot_required ? '已保存，可点「重启设备」应用' : '已保存', 'success');
  } catch (err) {
...
export const networkPage = {
  async mount(root) {
    try {
      const [cfg, status] = await Promise.all([api.getNetwork(), api.getStatus().catch(() => null)]);
      if (status) root.append(statusCard(status));
      root.append(formCard(cfg, {
        onScan:   handleScan,
